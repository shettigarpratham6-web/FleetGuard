...
import * as auditController from './src/controllers/auditController';
...
describe('Audit Controller', () => {
  ...
  test('getAuditLogs - should return 200 and list of audit logs', async () => {
    (db.query as any).mockResolvedValueOnce({ rows: [{ id: 1, action: 'LOGIN' }] });
    const req: any = { query: {} };
    const res = mockRes();
  ...
  test('getAuditLogsByEntity - should return 200 and entity logs', async () => {
    (db.query as any).mockResolvedValueOnce({ rows: [{ id: 2, entity_type: 'VEHICLE' }] });
    const req: any = { params: { entityType: 'VEHICLE', entityId: '1' } };
    const res = mockRes();
...
describe('Branch Controller', () => {
  ...
  test('createBranch - should return 201 when branch is created successfully', async () => {
    (db.query as any).mockResolvedValueOnce({ rows: [{ id: 10, branch_name: 'Main Branch' }] });
    const req: any = { body: { branch_name: 'Main Branch' } };
    const res = mockRes();
  ...
  test('getAllBranches - should return 200 and list of branches', async () => {
    (db.query as any).mockResolvedValueOnce({ rows: [{ id: 1, branch_name: 'HQ Branch' }] });
    const req: any = { query: {} };
    const res = mockRes();
