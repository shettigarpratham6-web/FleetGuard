 ...
 */
exports.getAlertSettings = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
 ...
 */
exports.updateAlertSettings = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (
      lead_days &&
      !Array.isArray(lead_days)
    ) {
      return res.status(400).json({
        error:
          'lead_days must be an array of numbers'
      });
    }
 ...
 */
exports.triggerExpiryScan = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    res.status(200).json({
      message:
        'Expiry scan executed successfully',
      details: result
    });
  } catch (error) {
    next(error);
