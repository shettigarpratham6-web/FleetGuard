...
exports.getAllRisks = async (
  ...
  try {
    ...
    if (typeof risk_level === 'string') {
      params.push(risk_level);
      conditions.push(
        `mr.risk_level = $${params.length}`
      );
    }
    ...
    queryText +=
      ' ORDER BY mr.remaining_distance ASC';
...
exports.getRiskByVehicleId = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
exports.triggerRecalculation = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    if (vehicle_id) {
      ...
      return res.status(200).json({
        message:
          'Maintenance risk calculated successfully',
        risk
      });
    } else {
