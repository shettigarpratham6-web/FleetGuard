...
exports.getAllBranches = async (
  ...
  try {
    ...
    if (typeof search === 'string') {
      ...
      conditions.push(
        `(branch_name ILIKE $${params.length} OR manager_name ILIKE $${params.length} OR city ILIKE $${params.length})`
      );
    }
...
exports.deleteBranch = async (
  ...
  try {
    ...
    const checkVehiclesText =
      'SELECT COUNT(*) FROM vehicles WHERE branch_id = $1';

    const checkVehiclesResult = await db.query(
      checkVehiclesText,
      [id]
    );

    const vehicleCount = parseInt(
      checkVehiclesResult.rows[0].count,
      10
    );
