...
function logStep(stepNum: number, description: string): void {
...
function finish(passed: number, total: number): void {
...
async function runCheck(): Promise<void> {
  ...
  try {
    ...
    if (listRes.status === 200 && Array.isArray(listData.notifications)) {
      const found = listData.notifications.some((n: any) => n.id === notificationId);
      if (found) {
