...
export default function FleetManagersPage() {
  ...
  useEffect(() => {
    const fetchManagers = async () => {
      try {
        const data = await api.auth.getUsers('Fleet Manager');
        setManagers(data || []);
      } catch (err) {
        console.error("Failed to fetch fleet managers", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <LayoutWrapper>
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
        <div className="border-b border-slate-200 pb-4">
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Fleet Managers</h2>
          <p className="text-slate-500 text-sm mt-1">Manage personnel overseeing fleet operations.</p>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {managers.length === 0 && (
                  <tr><td colSpan={4} className="p-8 text-center text-slate-500">No Fleet Managers registered.</td></tr>
                )}
              </tbody>
            </table>
          </div>
